...
static const char crc8_tab[256] = {0x0, 0x1d, 0x3a, 0x27, 0x74, 0x69, 0x4e, 0x53,
...
static const char crc8H2F_tab[256] =  {0x0, 0x2f, 0x5e, 0x71, 0xbc, 0x93, 0xe2, 0xcd,
...
{
    unsigned char  remainder = start;
    unsigned char  bit;
    unsigned char  topbit = 0x80;
    ...
    for (unsigned long byte = 0; byte < nBytes; byte++) {
        ...
        for (bit = 8; bit > 0; bit--) {
            ...
            if (remainder & topbit) {
                remainder = (unsigned char)(remainder << 1) ^ poly;
            }
            else {
...
unsigned char Crc_CalculateCRC8(unsigned char* Crc_DataPtr, unsigned long Crc_Length, unsigned char Crc_StartValue8, unsigned char Crc_IsFirstCall )
{
    ...
    if (Crc_DataPtr != 0) 
    {
        ...
        for (unsigned long byte = 0; byte < Crc_Length; byte++) 
        {
